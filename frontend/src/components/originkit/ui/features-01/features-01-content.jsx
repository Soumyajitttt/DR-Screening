...
const A = "/originkit/features-01";

/*
 * Feature set mirrors the four core stages of the RetinaVision AI workflow
 * (see LandingPage's FLOW_CARDS: Patient Database -> Screening & Analysis ->
 * Diagnostic AI & Grad-CAM -> Specialist Referral). Layout, motion, spacing,
 * shadows and typography are ported 1:1 from the Originkit features-01
 * export — only copy + icon/image pairing were adapted to fit the product.
 */
const FEATURES = [
  {
    id: "database",
    title: "Patient Database",
    description: "Registry with smart filters",
    icon: (
      <svg
  ...
  },
  {
    ...
    description: "Upload, review, done in minutes",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
    ...
    image: `${A}/planning.png`,
  },
  {
    id: "diagnosis",
    title: "Diagnostic AI & Grad-CAM",
    description: "Explainable grading, not a black box",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
  ...
  },
  {
    ...
    description: "Encrypted reports, sent instantly",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
    ...
    image: `${A}/powerful-integration.png`,
  },
];
...
const FeatureTabVisual = ({ feature, variant }) => {
...
const FeatureTabs = ({ activeId, onSelect, onKeyDown }) => {
  return (
    <ScrollReveal
      as="div"
      role="tablist"
      aria-label="Screening workflow features"
...
const Features01Content = () => {
  const [activeId, setActiveId] = useState("screening");

  const handleSelectFeature = (nextId) => {
        ...
        <div className="relative z-10 flex w-full flex-col items-center gap-10 ipad:gap-15">
          <header className="flex w-full max-w-180 flex-col items-center gap-3 text-center">
            <ScrollReveal
              ...
              blur={10}
              y={24}
              className="text-wrap text-[clamp(2rem,4vw,2.5rem)] font-medium leading-[1.12] tracking-[-0.04em] text-[#010110]"
            >
              One Platform for the Full Screening Workflow
            </ScrollReveal>
            <ScrollReveal
              as="p"
              blur={10}
              y={24}
              delay={0.08}
              className="max-w-155 text-pretty text-[16px] font-medium leading-normal tracking-[-0.02em] text-[#45545e] iphone:text-base"
            >
              From patient intake to specialist referral, RetinaVision AI
              <br className="hidden ipad:block" /> keeps every step
              connected, explainable, and fast.
            </ScrollReveal>
          </header>
          ...
          <ScrollReveal
            ...
            y={40}
            delay={0.16}
            className={`flex w-full max-w-237.5 flex-col gap-5 overflow-clip rounded-3xl bg-white p-3 ${CARD_SHADOW} ipad-landscape:flex-row ipad-landscape:items-stretch ipad-landscape:gap-5`}
          >
            <FeatureTabs
              activeId={activeId}
...
export default Features01Content;
