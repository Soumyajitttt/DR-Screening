...
const MarqueeHalf = ({ logos, duplicate = false }) => (
  <ul className="flex shrink-0 items-center gap-x-6 pr-6 ipad:gap-x-10 ipad:pr-10" aria-hidden={duplicate || undefined}>
...
const TrustedBy = ({ logos, active = false }) => {
  ...
  return (
    <motion.div
      ...
      transition={{ type: "tween", duration: prefersReducedMotion ? 0.2 : 0.35, ease: easeOutCubic }}
    >
      <div className="flex w-full max-w-150 items-center gap-3 ipad:gap-4">
        <span aria-hidden="true" className="h-px min-w-0 flex-1 bg-linear-to-r from-[#4b4b4b]/0 to-[#4b4b4b]" />
        <p className="shrink-0 text-center text-[16px] font-medium tracking-[-0.0125em] text-black/70 ipad:text-base">
          More than 100+
          <br className="block ipad:hidden" /> health centers trust us
        </p>
        <span aria-hidden="true" className="h-px min-w-0 flex-1 bg-linear-to-r from-[#4b4b4b] to-[#4b4b4b]/0" />
      </div>
